...
// Show HTTPS notice if camera won't work
window.addEventListener('DOMContentLoaded',()=>{
  const isSecure=location.protocol==='https:'||location.hostname==='localhost'||location.hostname==='127.0.0.1';
  const notice=document.getElementById('httpsNotice');
  if(!isSecure&&notice)notice.style.display='block';
});
...
function setCamBtnState(state){
  const btn=document.getElementById('camToggleBtn');
  if(!btn)return;
  if(state==='loading'){btn.textContent='⏳ Starting camera...';btn.disabled=true;btn.style.opacity='0.7';}
  else if(state==='on'){btn.textContent='🔴 Stop Camera';btn.disabled=false;btn.style.opacity='1';btn.style.background='rgba(231,76,60,.18)';btn.style.borderColor='rgba(231,76,60,.5)';}
  else{btn.textContent='📷 Start Camera';btn.disabled=false;btn.style.opacity='1';btn.style.background='';btn.style.borderColor='';}
}
...
async function startCamera(){
  ...
  if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia){
    const msg='Camera API not available. Ensure the page is served over HTTPS (required for camera access on mobile).';
    if(statusEl){statusEl.style.display='block';statusEl.style.color='#e74c3c';statusEl.textContent='✗ '+msg;}
    addAlert('warn','📷',msg,nowUTC());
    setCamBtnState('off');
    return;
  }

  setCamBtnState('loading');
  if(statusEl){statusEl.style.display='block';statusEl.style.color='var(--t2)';statusEl.textContent='⏳ Requesting camera permission...';}
  ...
  if(!stream){
    const errMsg=lastErr?.name==='NotAllowedError'||lastErr?.name==='PermissionDeniedError'
      ?'Camera permission denied. Go to browser Settings → Site Permissions → Camera → Allow, then try again.'
      :`Camera unavailable: ${lastErr?.message||'unknown error'}`;
    if(statusEl){statusEl.style.display='block';statusEl.style.color='#e74c3c';statusEl.textContent='✗ '+errMsg;}
    addAlert('warn','📷',errMsg,nowUTC());
    setCamBtnState('off');
    return;
  }
  ...
  try{await video.play();}catch(e){/* autoplay policy — video may still render */}

  container.style.display='block';
  document.getElementById('camOffsetRow')&&(document.getElementById('camOffsetRow').style.display='flex');
  setCamBtnState('on');
  if(statusEl){statusEl.style.display='block';statusEl.style.color='#27ae60';statusEl.textContent='✓ Camera active — aim crosshair at your survey mark, then press PICK POINT';}
  addAlert('ok','📷','Camera active — crosshair ready',nowUTC());
}
